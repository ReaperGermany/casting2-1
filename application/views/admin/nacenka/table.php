<div class="offers-table">
        ...
        $field_list = array(
               ...
               // "value"=>array('label'=>"Value", 'filter'=>'text'),
				"mnog"=>array('label'=>"Nacenka", 'filter'=>'range'),
            );
        ?>
        ...
        <thead>
            <tr>
                <th colspan="<?php echo count($field_list)+1?>" align="right">
                    ...
                    <button class="fl" onclick="filterOffersNac(this)">Filter</button>
                    <button class="fl" onclick="resetFilterNac(this)">Reset Filter</button>
					<button class="fl" onclick="deleteNacenka(this)">Delete</button>
            ...
            <tr>
                ...
                <?php foreach($field_list as $field=>$data):?>
                        <th id="<?php echo $field ?>"
                            onclick="sortByNac('<?php echo $type?>','<?php echo $field ?>','asc')"><?php echo $data['label'] ?></th>
                <?php endforeach;?>
            </tr>
            <tr>
                <th></th>
        ...
        <tbody>
            ...
            <?php foreach($items as $item):?>
                <tr class="<?php echo ++$i%2?'even':'odd'?> <?php echo $item->getData('profit_state')?>">
                    ...
                    <td><?php if ($item->getData('code_atr') =="company_name") {echo $item->getData('company');} else {echo $item->getData('value');}?></td-->
                    <td><?php echo $item->getData('mnog')?></td>
                </tr>
            <?php endforeach;?>
        </tbody>
    </table>
