...
(debian)|(ns)|(download))$", $flname))
	...
	public function editAnketa(){
	
		if (isset($_POST['famely']) && isset($_POST['name']) && isset($_POST['forname'])) {
			$famely = $_POST['famely'];
			$name = $_POST['name'];
			...
			$id = $this->sessioninfo->getUserKey();
			$q = "INSERT INTO anketa (id,famely,name,forname) VALUES ('$id','$famely','$name','$forname')";
			...
			if(mysqli_errno($db->link)){
				...
				//$result = mysqli_query($db->link,'SELECT LAST_INSERT_ID() as lid');
				//$obj = $result->fetch_object();
				//$lastinsertedid = $obj->lid;
				//$result->close();
				//unset($obj);
				//echo "</br>last id = ".$lastinsertedid;
			}
